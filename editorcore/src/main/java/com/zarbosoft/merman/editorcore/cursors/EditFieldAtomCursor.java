...
public class EditFieldAtomCursor extends FieldAtomCursor {
  public EditFieldAtomCursor(Context context, VisualFrontAtomBase base) {
    super(context, base);
  }

  public void editCut(Editor editor) {
    editor.context.copy(Context.CopyContext.ARRAY, TSList.of(visual.atomGet()));
    editor.history.record(
        editor.context,
        null,
        recorder -> {
          editor.atomSet(
              editor.context, recorder, visual, editor.createEmptyGap(editor.context.syntax.gap));
        });
  }
    ...
    editor.context.uncopy(
        ...
        atoms -> {
          ...
          editor.history.record(
              ...
              recorder -> {
                if (atoms.size() == 1) {
                  editor.atomSet(editor.context, recorder, visual, atoms.get(0));
                } else {
                  Atom gap = editor.createEmptyGap(editor.context.syntax.suffixGap);
                  editor.atomSet(editor.context, recorder, visual, gap);
                  recorder.apply(
                      editor.context,
                      new ChangeArray(
                          (FieldArray) gap.fields.get(BaseGapAtomType.PRIMITIVE_KEY), 0, 0, atoms));
                }
              });
        });
  }
