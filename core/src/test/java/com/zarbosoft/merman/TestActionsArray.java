...
public class TestActionsArray {
  ...
  public void testEnter() {
    ...
    FreeAtomType snooze =
        new TypeBuilder("snooze")
            .back(
                new BackArraySpec(
                    new BaseBackArraySpec.Config("value", infinity.id(), TSList.of())))
            .frontDataArray("value")
            .build();
    Syntax syntax = new SyntaxBuilder(snooze.id()).type(infinity).type(snooze).build();
    final Context context =
        build(
            syntax,
            new TreeBuilder(snooze).addArray("value", new TreeBuilder(infinity).build()).build());
    visual(context).select(context, true, 0, 0);
    Helper.cursorArray(context).actionEnter(context);
    assertThat(
        context.cursor.getSyntaxPath(),
        equalTo(
            new SyntaxPath("named", "value", "0", "named", "value", "0", "named", "value", "0")));
  }

  public Context build(Syntax syntax, final Atom... atoms) {
    final Context context =
        buildDoc(syntax, new TreeBuilder(MiscSyntax.array).addArray("value", atoms).build());
  ...
  public void testNextEnd() {
    ...
    assertThat(
        context.cursor.getSyntaxPath(),
        equalTo(new SyntaxPath("named", "value", "0", "named", "value", "0")));
  }
  ...
  public void testPreviousStart() {
    ...
    assertThat(
        context.cursor.getSyntaxPath(),
        equalTo(new SyntaxPath("named", "value", "0", "named", "value", "4")));
  }
