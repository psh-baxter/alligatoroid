...
public class ROSet<T> implements ROSetRef<T> {
  ...
  @Override
  public ROSet<T> union(ROSetRef<T> other) {
    return new ROSet<T>(Sets.union(data, other.inner_()));
  }

  @Override
  public ROSet<T> intersect(ROSetRef<T> other) {
    return new ROSet<T>(Sets.intersection(data, other.inner_()));
  }
