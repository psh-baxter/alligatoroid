...
public class JavaFXDisplay extends Display {
  ...
  public JavaFXDisplay(final Syntax syntax) {
    super(syntax.converseDirection, transverseDirection);
    ...
    final ChangeListener<Number> converseSizeListener =
        (observable, oldValue, newValue) -> {
          ...
          if (newValue1 > oldConverseEdge && newValue1 < oldConverseEdge + 10) return;
          ImmutableList.copyOf(converseEdgeListeners)
              .forEach(
                  l -> {
    ...
    final ChangeListener<Number> transverseSizeListener =
        (observable, oldValue, newValue) -> {
          final int newValue1 = (int) newValue.doubleValue();
          // JavaFX does something stupid where middle mouse wheel causes the node to resize by 1px
          if (newValue1 > oldTransverseEdge && newValue1 < oldTransverseEdge + 10) return;
          ImmutableList.copyOf(transverseEdgeListeners)
              .forEach(l -> l.changed(oldTransverseEdge, newValue1));
  ...
  public Vector sceneToVector(final Syntax syntax, final double x, final double y) {
    int converse = 0;
    int transverse = 0;
